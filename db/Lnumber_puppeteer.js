...
async function loginAndScrapeSeniorInfo() {
    ...
    try {
      ...
      const USERNAME = 'dongnebangne1@naver.com'; 
      const PASSWORD = 'ksh0110!!!';

      // 아이디와 비밀번호 입력 필드가 나타날 때까지 기다림
      await page.waitForSelector('#mb_email');

      // 아이디와 비밀번호 입력
      await page.type('#mb_email', USERNAME);
      await page.type('#mb_password', PASSWORD);
      ...
      console.log('로그인 성공! 다음 페이지로 이동합니다...');
      // --- 자동 로그인 종료 ---
