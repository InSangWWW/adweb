...
async function getSubCategories() {
  try {
    ...
    for (const category of subCategories) {
      try {
        // 이미 존재하는 카테고리인지 확인
        let existingCategory = await WelfareCategory.findOne({ caId: category.caId });
        ...
        if (existingCategory) {
          ...
          // 기존 카테고리 ID 설정
          existingCategory = await WelfareCategory.findOne({ caId: category.caId });
        ...
        console.log(`카테고리 '${category.name}'의 ID: ${category._id}`);
      } catch (error) {
        console.error(`카테고리 '${category.name}' 저장 중 오류:`, error);
      }
    }
...
// 특정 페이지의 상품 URL 추출 함수
async function getProductUrlsFromPage(categoryUrl, page = 1) {
  try {
    // 페이지 파라미터 추가
    const pageUrl = `${categoryUrl}${categoryUrl.includes('?') ? '&' : '?'}page=${page}`;
    console.log(`페이지 스크래핑 시작: ${pageUrl}`);
    ...
    console.log(`페이지 ${page}에서 ${productUrls.length}개 상품 URL 추출 완료`);
    ...
    console.error(`페이지 ${page} 스크래핑 오류:`, error.message);
    return {
      productUrls: [],
      hasNextPage: false
    };
...
async function getAllProductUrlsFromCategory(categoryUrl) {
  ...
  while (hasNextPage) {
    const { productUrls, hasNextPage: nextPageExists } = await getProductUrlsFromPage(categoryUrl, page);
    
    allProductUrls.push(...productUrls);
    
    if (!nextPageExists || productUrls.length === 0) {
      hasNextPage = false;
    } else {
      page++;
      // 서버 부하 방지를 위한 지연
  ...
  console.log(`카테고리에서 총 ${allProductUrls.length}개 상품 URL 추출 완료`);
...
async function scrapeProductDetail(url, categoryId) {
  try {
    console.log(`${url} 스크래핑 시작... (카테고리 ID: ${categoryId})`);
    ...
    console.log('카테고리 ID 확인:', item.categoryId);
...
async function scrapeSubCategory(subCategory) {
  try {
    // 1. 카테고리의 모든 페이지에서 상품 URL 가져오기
    const productUrls = await getAllProductUrlsFromCategory(subCategory.url);
...
async function main() {
  try {
    ...
    const targetSubCategory = subCategories.find(cat => cat.name.includes('이동변기'));
    ...
    // 모든 서브 카테고리 스크래핑 (주석 해제하여 사용)
  
    for (const subCategory of subCategories) {
      console.log(`'${subCategory.name}' 서브 카테고리 처리 중...`);
      
      const products = await scrapeSubCategory(subCategory);
      console.log(`${products.length}개 상품 스크래핑 및 저장 완료`);
